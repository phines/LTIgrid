function [ps,Pgs_sbs] = Econ_Dispatch_fn(ps,Load,perc_reg)
...
for j=1:num_areas
    ...
    PgSS_area      = PgSS(gen_areas==j);
    num_gens       = size(gendata_area,1);
    ...
    reg_per_gen    = reg_scheduled/num_gens;
...
%% Setup inputs
    ...
    for k=1:num_time_steps 
        ...
        curr_a = (1:num_gens)+(k-1)*(num_gens+2); %+2?
    ...
    cols_2        = cols+(num_gens+2);
    
    if num_time_steps==1
        A=[];
        b=[];
    else
        A_ru  = sparse(rows,cols(rows),-1,(num_time_steps-1)*num_gens,num_time_steps*(num_gens+2));
        A_ru  = A_ru + sparse(rows,cols_2(rows),1,(num_time_steps-1)*num_gens,num_time_steps*(num_gens+2));
        A_rd  = sparse(rows,cols(rows),1,(num_time_steps-1)*num_gens,num_time_steps*(num_gens+2));
        A_rd  = A_rd + sparse(rows,cols_2(rows),-1,(num_time_steps-1)*num_gens,num_time_steps*(num_gens+2));
        A = [A_ru;A_rd];
    ...
    for k = 1:num_time_steps
        ...
        Aeq      = Aeq + sparse(k,curr_loc_top,+1,num_time_steps, (num_gens+2)*(num_time_steps));
    end
    ...
    for i=1:num_gens
       count = count+1;
       Pgs_sbs(:,count) = Pgs(i:num_gens+2:length(Pgs));
    end

    S_plus = Pgs(num_gens+1:num_gens+2:end);
    S_minus = Pgs(num_gens+2:num_gens+2:end);
